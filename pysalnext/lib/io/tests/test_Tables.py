...
class Test_Table(ut.TestCase):
    def setUp(self):
        self.filehandler = psopen(pysalnext_examples.get_path('columbus.dbf'))
        self.df = self.filehandler.to_df()
        self.filehandler.seek(0)
        self.shapefile = psopen(pysalnext_examples.get_path('columbus.shp'))
        self.csvhandler = psopen(pysalnext_examples.get_path('usjoin.csv'))
