...
def south(df=False):
    ...
    """
    import geopandas
    data = geopandas.read_file(pysalnext.lib.examples.get_path('south.shp'))
    ...
    W2 = pysalnext.lib.weights.user.queen_from_shapefile(pysalnext.lib.examples.get_path('us48.shp'),
                                 idVariable='STATE_NAME')
    W2 = pysalnext.lib.weights.Wsets.w_subset(W2, ids=data.STATE_NAME.unique().tolist()) #only keep what's in the data
    W1 = pysalnext.lib.weights.user.queen_from_shapefile(pysalnext.lib.examples.get_path('south.shp'),
                                 idVariable='FIPS')
    W1 = pysalnext.lib.weights.Wsets.w_subset(W1, ids=data.FIPS.tolist()) #again, only keep what's in the data
...
def baltim(df=False):
    ...
    """
    import geopandas
    baltim = geopandas.read_files(pysalnext.lib.examples.get_path('baltim.shp'))
