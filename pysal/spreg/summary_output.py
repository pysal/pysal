...
def TSLS(reg, vm, w, spat_diag):
    ...
    summary(reg=reg, vm=vm, instruments=True, nonspat_diag=False,
            spat_diag=spat_diag)
...
def GM_Error(reg, vm, w):
    ...
    summary(reg=reg, vm=vm, instruments=False, nonspat_diag=False,
            spat_diag=False)
...
def Probit(reg, vm, w, spat_diag):
    ...
    summary(reg=reg, vm=vm, short_intro=True, instruments=False,
            nonspat_diag=False, spat_diag=spat_diag, other=True)
...
def summary_nonspat_diag_1(reg):
    ...
    strSummary += "%-20s:%12.4f  %-22s:%12.3f\n" % ('S.E of regression ML',
                                                    np.sqrt(reg.sig2ML), 'Schwarz criterion', reg.schwarz)
    return strSummary
