...
class TestBaseGMLag(unittest.TestCase):
    ...
    def test___init__(self):
        ...
        yd2, q2 = pysal.spreg.utils.set_endog(
            self.y, self.X, self.w, None, None, 2, True)
    ...
    def test_init_white_(self):
        ...
        base_gm_lag = BaseGM_Lag(self.y, self.X, yend=yd2, q=q2,
                                 w=self.w, w_lags=2, robust='white')
    ...
    def test_init_hac_(self):
        ...
        base_gm_lag = BaseGM_Lag(self.y, self.X, yend=yd2, q=q2,
                                 w=self.w, w_lags=2, robust='hac', gwk=gwk)
        tbetas = np.array([[4.53017056e+01], [6.20888617e-01], [-
                                                                4.80723451e-01], [2.83622122e-02]])
        np.testing.assert_array_almost_equal(base_gm_lag.betas, tbetas)
        dbetas = D.se_betas(base_gm_lag)
    ...
    def test_init_discbd(self):
        ...
        q = np.reshape(q, (49, 1))
        yd2, q2 = pysal.spreg.utils.set_endog(
            self.y, self.X, self.w, yd, q, 2, True)
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        reg = BaseGM_Lag(self.y, self.X, yend=yd2, q=q2, w=self.w, w_lags=2)
    ...
    def test_lag_q(self):
        ...
        q = np.reshape(q, (49, 1))
        yd2, q2 = pysal.spreg.utils.set_endog(
            self.y, self.X, self.w, yd, q, 2, False)
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        reg = BaseGM_Lag(self.y, self.X, yend=yd2, q=q2, w=self.w,
                         w_lags=2, lag_q=False)
