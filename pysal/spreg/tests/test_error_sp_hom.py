...
class BaseGM_Error_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        ...
        np.testing.assert_array_almost_equal(
            reg.predy[0], np.array([53.000269]), 6)
...
class GM_Error_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        ...
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        np.testing.assert_array_almost_equal(
            reg.u[0], np.array([27.46673388]), 6)
        np.testing.assert_array_almost_equal(
            reg.e_filtered[0], np.array([32.37298547]), 7)
        np.testing.assert_array_almost_equal(
            reg.predy[0], np.array([53.00026912]), 6)
        self.assertAlmostEquals(reg.n, 49, 7)
        self.assertAlmostEquals(reg.k, 3, 7)
        vm = np.array([[1.51340717e+02, -5.29057506e+00, -1.85654540e+00, -2.39139054e-03], [-5.29057506e+00, 2.46669610e-01, 5.14259101e-02, 3.19241302e-04], [-1.85654540e+00, 5.14259101e-02, 3.20510550e-02, -5.95640240e-05], [-2.39139054e-03, 3.19241302e-04, -5.95640240e-05, 3.36690159e-02]])
        np.testing.assert_array_almost_equal(reg.vm, vm, 6)
        ...
        self.assertAlmostEqual(reg.sig2, sig2)
        std_err = np.array(
            [12.30206149, 0.49665844, 0.17902808, 0.18349119])
...
class BaseGM_Endog_Error_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        ...
        betas = np.array([[55.36575166], [0.46432416], [-
                                                        0.66904404], [0.43205526]])
        ...
        sig2 = 0
        #self.assertAlmostEqual(reg.sig2,sig2)
        hth = np.array([[49., 704.371999, 139.75], [704.371999, 11686.67338121,
...
class GM_Endog_Error_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        ...
        #std_err
        std_err = np.array(
            [23.49604343, 0.73817223, 0.39433722, 0.19267128])
        np.testing.assert_array_almost_equal(reg.std_err, std_err, 6)
        z_stat = np.array([[2.35638617, 0.01845372], [0.62901874, 0.52933679],
                           [-1.69662923, 0.08976678], [2.24244556, 0.02493259]])
        np.testing.assert_array_almost_equal(reg.z_stat, z_stat, 6)
...
class BaseGM_Combo_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        yd2, q2 = pysal.spreg.utils.set_endog(
            self.y, self.X, self.w, None, None, 1, True)
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        reg = HOM.BaseGM_Combo_Hom(
            self.y, self.X, yend=yd2, q=q2, w=self.w, A1='hom_sc')
        np.testing.assert_array_almost_equal(
            reg.y[0], np.array([80.467003]), 7)
        x = np.array([1., 19.531])
        np.testing.assert_array_almost_equal(reg.x[0], x, 7)
        betas = np.array(
            [[10.12541428], [1.56832263], [0.15132076], [0.21033397]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        np.testing.assert_array_almost_equal(
            reg.u[0], np.array([34.3450723]), 7)
        np.testing.assert_array_almost_equal(
            reg.e_filtered[0], np.array([36.6149682]), 7)
        np.testing.assert_array_almost_equal(
            reg.predy[0], np.array([46.1219307]), 7)
        self.assertAlmostEquals(reg.n, 49, 7)
        self.assertAlmostEquals(reg.k, 3, 7)
...
class GM_Combo_Hom_Tester(unittest.TestCase):
    ...
    def test_model(self):
        ...
        np.testing.assert_array_almost_equal(
            reg.e_filtered[0], np.array([36.6149682]), 7)
        np.testing.assert_array_almost_equal(
            reg.e_pred[0], np.array([32.90372983]), 7)
        np.testing.assert_array_almost_equal(
            reg.predy[0], np.array([46.1219307]), 7)
        np.testing.assert_array_almost_equal(
            reg.predy_e[0], np.array([47.56327317]), 7)
        self.assertAlmostEquals(reg.n, 49, 7)
        self.assertAlmostEquals(reg.k, 3, 7)
        z = np.array([1., 19.531, 35.4585005])
        np.testing.assert_array_almost_equal(reg.z[0], z, 7)
        ...
        self.assertAlmostEqual(reg.sig2, sig2)
        std_err = np.array(
            [15.28707761, 0.44072838, 0.40479714, 0.42263726])
        np.testing.assert_array_almost_equal(reg.std_err, std_err, 6)
