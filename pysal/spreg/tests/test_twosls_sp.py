...
class TestGMLag(unittest.TestCase):
    ...
    def test_reg(self):
        reg = pysal.spreg.twosls_sp.GM_Lag(self.y, self.X, w=self.w, w_lags=2, name_x = ['inc', 'crime'], name_y = 'hoval', name_ds = 'columbus')
        tbetas = np.array([[ 4.53017056e+01], [  6.20888617e-01], [ -4.80723451e-01], [  2.83622122e-02]])
        np.testing.assert_array_almost_equal(tbetas, reg.betas)
    ...
    def test_white(self):
        ...
        robust = 'white', name_x = ['inc', 'crime'], name_y = 'hoval', name_ds = 'columbus')
        tbetas = np.array([[ 4.53017056e+01], [  6.20888617e-01], [ -4.80723451e-01], [  2.83622122e-02]])
        np.testing.assert_array_almost_equal(tbetas, reg.betas)

        se = np.array([20.47077481,   0.50613931,   0.20138425,   0.38028295 ] )
        np.testing.assert_array_almost_equal(reg.std_err, se)

    def test_discbd(self):
        X = np.array(self.db.by_col("INC"))
        X = np.reshape(X, (49,1))
        yd = np.array(self.db.by_col("CRIME"))
        yd = np.reshape(yd, (49,1))
        q = np.array(self.db.by_col("DISCBD"))
        q = np.reshape(q, (49,1))
        reg = pysal.spreg.twosls_sp.GM_Lag(self.y, X, w=self.w, yend=yd, q=q, w_lags=2, name_x=['inc'], name_y='hoval', name_yend=['crime'], name_q=['discbd'], name_ds='columbus')
