...
class TestBaseTSLS(unittest.TestCase):
    ...
    def test_basic(self):
        reg = pysal.spreg.twosls.BaseTSLS(self.y, self.X, self.yd, self.q)
    ...
    def test_n_k(self):
        reg = pysal.spreg.twosls.BaseTSLS(self.y, self.X, self.yd, self.q, sig2n_k=True)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
    ...
    def test_white(self):
        reg = pysal.spreg.twosls.BaseTSLS(self.y, self.X, self.yd, self.q, robust='white')
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
    ...
    def test_hac(self):
        ...
        reg = pysal.spreg.twosls.BaseTSLS(self.y, self.X, self.yd, self.q, robust='hac', gwk=gwk)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
...
class TestTSLS(unittest.TestCase):
    ...
    def test_basic(self):
        reg = pysal.spreg.twosls.TSLS(self.y, self.X, self.yd, self.q)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        h_0 = np.array([  1.   ,  19.531,   5.03 ])
        np.testing.assert_array_almost_equal(reg.h[0], h_0)
    ...
    def test_n_k(self):
        reg = pysal.spreg.twosls.TSLS(self.y, self.X, self.yd, self.q, sig2n_k=True)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        vm = np.array([[ 243.99486949,   11.04572682,  -10.16711028],
                       [  11.04572682,    2.13183469,   -1.08467261],
    ...
    def test_white(self):
        reg = pysal.spreg.twosls.TSLS(self.y, self.X, self.yd, self.q, robust='white')
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        vm = np.array([[ 208.27139316,   15.6687805 ,  -11.53686154],
                       [  15.6687805 ,    2.26882747,   -1.30312033],
    ...
    def test_hac(self):
        ...
        reg = pysal.spreg.twosls.TSLS(self.y, self.X, self.yd, self.q, robust='hac', gwk=gwk)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
    ...
    def test_spatial(self):
        ...
        reg = pysal.spreg.twosls.TSLS(self.y, self.X, self.yd, self.q, spat_diag=True, w=w)
        betas = np.array([[ 88.46579584], [  0.5200379 ], [ -1.58216593]])
        np.testing.assert_array_almost_equal(reg.betas, betas, 7)
        vm = np.array([[ 229.05640809,   10.36945783,   -9.54463414],
                       [  10.36945783,    2.0013142 ,   -1.01826408],
