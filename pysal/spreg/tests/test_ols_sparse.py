...
class TestBaseOLS(unittest.TestCase):
    ...
    def test_OLS(self):
        ...
        np.testing.assert_array_almost_equal(ols.betas,
                                             np.array(
                                                 [[46.42818268], [0.62898397],
                                                  [-0.48488854]]), 7)
        bp = np.array([2, 5.7667905131212587, 0.05594449410070558])
        ols_bp = np.array([ols.breusch_pagan['df'],
                           ols.breusch_pagan['bp'], ols.breusch_pagan['pvalue']])
        ...
        ols_jb = np.array([ols.jarque_bera['df'], ols.jarque_bera[
            'jb'], ols.jarque_bera['pvalue']])
        np.testing.assert_array_almost_equal(ols_jb, jb, 7)
        white = np.array([5, 2.90606708, 0.71446484])
        ols_white = np.array(
            [ols.white['df'], ols.white['wh'], ols.white['pvalue']])
