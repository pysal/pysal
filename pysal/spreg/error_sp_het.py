...
class BaseGM_Endog_Error_Het(RegressionPropsY):
    ...
    def __init__(self, y, x, yend, q, w,
        ...
        while self.iteration < max_iter and eps > epsilon:
            ...
            #2b. GMM --> \hat{\lambda}
            vc2 = get_vc_het_tsls(w, self, lambda_old,
        ...
        vc3 = get_vc_het_tsls(
            w, self, lambda3, P, zs, inv_method, save_a1a2=True)
...
def get_vm_het(G, lamb, reg, w, psi):
    ...
    vm = np.vstack(
        (np.hstack((omega11, zero)), np.hstack((zero.T, omega22)))) / w.n
    return vm
...
def get_a1a2(w, reg, lambdapar, P, zs, inv_method, filt):
    ...
    if not filt:
        a1 = UTILS.inverse_prod(w, a1, lambdapar,
                                post_multiply=True, inv_method=inv_method).T
        a2 = UTILS.inverse_prod(w, a2, lambdapar,
                                post_multiply=True, inv_method=inv_method).T
    return [a1, a2]
