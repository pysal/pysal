...
class BaseProbit:
    ...
    def scale(self):
        if 'scale' not in self._cache:
            ...
            if self.scalem == 'xmean':
                self._cache['scale'] = float(norm.pdf(
                    np.dot(self.xmean.T, self.betas)))
        return self._cache['scale']

    @property
    def slopes(self):
        if 'slopes' not in self._cache:
            self._cache['slopes'] = self.betas[1:] * \
                self.scale  # Disregard the presence of dummies.
    ...
    def slopes_vm(self):
        if 'slopes_vm' not in self._cache:
            ...
            slopes_vm = (self.scale ** 2) * np.dot(np.dot(dfdb,
                                                          self.vm), dfdb.T)
    ...
    def LR(self):
        if 'LR' not in self._cache:
            ...
            LR = float(-2 * (self.n * (P * np.log(
                P) + (1 - P) * np.log(1 - P)) - self.logl))
    ...
    def Pinkse_error(self):
        if 'Pinkse_error' not in self._cache:
            self._cache['Pinkse_error'], self._cache['KP_error'], self._cache[
                'PS_error'] = sp_tests(self)
    ...
    def par_est(self):
        start = np.dot(la.inv(
            np.dot(self.x.T, self.x)), np.dot(self.x.T, self.y))
        ...
        if self.optim == 'newton':
            ...
            fgrad = lambda par: -self.gradient(par)
            if self.optim == 'bfgs':
                par_hat = op.fmin_bfgs(flogl, start, fgrad,
                                       full_output=1, disp=0)
                warn = par_hat[6]
            if self.optim == 'ncg':
                fhess = lambda par: -self.hessian(par)
                par_hat = op.fmin_ncg(flogl, start, fgrad,
                                      fhess=fhess, full_output=1, disp=0)
                warn = par_hat[5]
        if warn > 0:
            warn = True
        else:
