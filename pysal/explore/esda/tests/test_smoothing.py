...
from pysalnext.lib.weights.Distance import KNN, Kernel
from .. import smoothing as sm
import numpy as np
from pysalnext.lib.common import RTOL, ATOL, pandas
...
class TestSRate(unittest.TestCase):
    def setUp(self):
        sids = pysalnext.open(pysalnext.examples.get_path('sids2.dbf'), 'r')
        self.w = pysalnext.open(pysalnext.examples.get_path('sids2.gal'), 'r').read()
        ...
        self.stl = pysalnext.open(pysalnext.examples.get_path('stl_hom.csv'), 'r')
        self.stl_e, self.stl_b = np.array(self.stl[:, 10]), np.array(self.stl[:, 13])
        self.stl_w = pysalnext.open(pysalnext.examples.get_path('stl.gal'), 'r').read()
        if not self.stl_w.id_order_set:
            self.stl_w.id_order = list(range(1, len(self.stl) + 1))

        if not PANDAS_EXTINCT:
            self.df = pysalnext.open(pysalnext.examples.get_path('sids2.dbf')).to_df()
            self.ename = 'SID74'
            self.bname = 'BIR74'
            self.stl_df = pysalnext.open(pysalnext.examples.get_path('stl_hom.csv')).to_df()
...
class TestHB(unittest.TestCase):
    def setUp(self):
        sids = pysalnext.open(pysalnext.examples.get_path('sids2.shp'), 'r')
        self.sids = sids
        ...
        sids_db = pysalnext.open(pysalnext.examples.get_path('sids2.dbf'), 'r')
