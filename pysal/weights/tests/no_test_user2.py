...
from pysal.weights import user2 as u
from pysal.core.FileIO import FileIO as fopen

class Testuser(unittest.TestCase):
    def setUp(self):
        self.wq = u.Queen(fopen(pysal.examples.get_path("columbus.shp")))
        self.wr = u.Rook(fopen(pysal.examples.get_path("columbus.shp")))
    ...
    def test_knnW_from_shapefile(self):
        wc = u.Knn(fopen(pysal.examples.get_path("columbus.shp")))
        self.assertAlmostEquals(wc.pct_nonzero, 4.081632653061225)
        wc3 = u.Knn(fopen(pysal.examples.get_path( "columbus.shp")), k=3)
    ...
    def test_threshold_binaryW_from_shapefile(self):
        ids = fopen(pysal.examples.get_path('columbus.dbf')).by_col('POLYID')
        w = u.Threshold_Binary(fopen(pysal.examples.get_path("columbus.shp")),0.62, ids=ids)
        self.assertEquals(w.weights[1], [1, 1])

    def test_threshold_continuousW_from_array(self):
        points = [(10, 10), (20, 10), (40, 10), (15, 20), (30, 20), (30, 30)]
        wid = u.Threshold_Continuous(points, 11.2)
        ...
        wid2 = u.Threshold_Continuous(points, 11.2, alpha=-2.0)
    ...
    def test_threshold_continuousW_from_shapefile(self):
        ids = fopen(pysal.examples.get_path('columbus.dbf')).by_col('POLYID')
        w = u.Threshold_Continuous(fopen(pysal.examples.get_path("columbus.shp"))
                                , 0.62, ids=ids)
    ...
    def test_kernelW_from_shapefile(self):
        ids = fopen(pysal.examples.get_path('columbus.dbf')).by_col('POLYID')
        kw = u.Kernel(fopen(pysal.examples.get_path('columbus.shp')), ids=ids)
    ...
    def test_adaptive_kernelW(self):
        ...
        kwa = u.Kernel_Adaptive(points, bandwidths=bw)
        ...
        kweag = u.Kernel_Adaptive(points, function='gaussian')
    ...
    def test_adaptive_kernelW_from_shapefile(self):
        kwa = u.Kernel_Adaptive(fopen(pysal.examples.get_path('columbus.shp')))
        wds = {kwa.neighbors[0][i]: v for i, v in enumerate(kwa.weights[0])}
