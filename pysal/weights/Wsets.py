...
def w_union(w1, w2):
    ...
    >>> w1 = PW.weights.lat2gal(4,4)
    >>> w2 = PW.weights.lat2gal(6,4)
    >>> w = w_union(w1, w2)
    >>> w1[0] == w[0]
    True
    >>> w1.neighbors[15]
...
def w_difference(w1, w2, w_shape='w1', constrained=True):
    ...
    >>> w1 = PW.weights.lat2gal(4,4,rook=False)
    >>> w2 = PW.weights.lat2gal(4,4,rook=True)
...
def w_symmetric_difference(w1, w2, w_shape='all', constrained=True):
    ...
    >>> w1 = PW.weights.lat2gal(4,4,rook=False)
    >>> w2 = PW.weights.lat2gal(6,4,rook=True)
