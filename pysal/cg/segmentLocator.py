...
class BruteSegmentLocator(object):
    ...
    def nearest(self,pt):
        d = self.data
        distances = [get_segment_point_dist(d[i],pt)[0] for i in xrange(self.n)]
...
class SegmentGrid(object):
    ...
    def nearest(self, pt):
        ...
        if DEBUG:
            ...
            i = pylab.matshow(self.mask, origin='lower', extent=self.x_range+self.y_range, fignum=1)
...
def test_grid(bins, segments, qpoints, visualize=False):
    ...
    if visualize:
        i = pylab.matshow(G.grid.mask, origin='lower', extent=G.grid.x_range+G.grid.y_range)
...
def binSizeTest():
    q = 100
    minN = 1000; maxN = 10000; stepN = 1000
    minB = 250; maxB = 2000; stepB = 250
